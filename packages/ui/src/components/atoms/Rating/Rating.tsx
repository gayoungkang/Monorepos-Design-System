            ...
            >
                ...
                >
                    {Array.from({ length: max }).map((_, index) => {
                        const displayed =
                            hoverValue !== null ? hoverValue > index : currentValue > index;
...
const RatingItem = styled.div<{
  ...
  color:  ${({ theme, disabled }) => disabled  ? theme.colors.text.disabled : theme.colors.primary[400]};
  padding: 2px;
  line-height: 0;
  cursor: ${({ disabled, readOnly }) =>
        disabled || readOnly ? "no-drop" : "pointer"};
